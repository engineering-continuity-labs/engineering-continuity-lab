...
| ECL-AC-020 | **Given** a structurally valid analysis report from the loopback explorer service, **when** the browser explorer receives it, **then** it shows component, contributor, and departure views consistent with the report. |
| ECL-AC-021 | **Given** an incompatible or malformed report from the local service, **when** the browser explorer receives it, **then** it rejects it without accepting corrupted state. |
| ECL-AC-022 | **Given** the browser explorer, **when** it displays a report, **then** the application keeps report state only in browser memory and its static code contains no report upload behavior. |
...
| ECL-AC-035 | **Given** the local browser explorer, **when** it connects to its loopback service, **then** it visibly presents separate local-path and public-HTTPS clone-URL controls, and each uses the shared resolver without exposing a temporary workspace. |
...
| ECL-AC-038 | **Given** a visitor opens the hosted demo, **when** Explore Demo is selected, **then** the bundled synthetic report displays Overview, Components, Contributors, Evidence, and Continuity Stress Test. |
| ECL-AC-039 | **Given** a visitor opens the hosted demo, **when** Explore Demo is selected, **then** the bundled synthetic report is displayed with the standard dashboard views. |
| ECL-AC-040 | **Given** the hosted demo, **when** it is inspected, **then** it exposes no repository inputs, cloning controls, report-import controls, or filesystem access. |
| ECL-AC-041 | **Given** the hosted demo, **when** a visitor inspects its primary actions and disclaimer, **then** View on GitHub targets the project repository and the page visibly states that results are historical activity evidence rather than employee evaluation. |
...
| ECL-AC-044 | **Given** the primary browser workflow, **when** a user finishes viewing a report, **then** Open Analysis Report, Export Analysis Report, and Reset to Demo controls are not presented. |
...
| ECL-AC-046 | **Given** the loopback explorer without an initial source, **when** it loads, **then** repository selection is shown before any dashboard or synthetic report content. |
| ECL-AC-047 | **Given** successful local or public HTTPS analysis, **when** a report is received, **then** the dashboard appears and Analyze Another Repository returns to source selection. |
| ECL-AC-048 | **Given** a loaded report, **when** Overview renders, **then** it shows the four summary metrics, risk distribution, top five concentrated components, and a top-five-plus-Others contributor evidence-share visualization with a neutral disclaimer. |
| ECL-AC-049 | **Given** hosted Pages, **when** it loads, **then** it offers only the static demo journey and local-explorer instructions without functional repository controls. |
