...
| ECL-AC-038 | **Given** a visitor opens the hosted demo, **when** the page finishes loading, **then** the bundled dotnet/eShop Git-evidence sample is already displayed and Overview, Components, Contributors, Evidence, and Continuity Stress Test are available. |
...
| ECL-AC-051 | **Given** the hosted demo loads, **when** no local explorer is available, **then** it displays the bundled dotnet/eShop report at revision `b4a40872005d4bb29e5b1fa1ff7e244143d39215`, labels it as static Git-history evidence, and does not clone or contact the eShop repository. |
| ECL-AC-052 | **Given** a visitor sees a report-opening or sample-navigation action, **when** they read its label or focus the report action, **then** it identifies a saved JSON report or the eShop sample destination, does not imply a download, and exposes an accessible report-action explanation. |
| ECL-AC-053 | **Given** a report loads in the browser explorer, **when** the initial dashboard renders, **then** summary metrics are visible and the full-width Components view is active with compact table typography; Overview and detailed component evidence remain available. |
