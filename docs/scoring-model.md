...
## Effect of explicit filtering

Optional author/path filters remove changed-file records before calculating all six signals. Consequently churn totals, file coverage, unique contribution, frequency, and active months are recalculated from retained evidence. The reference timestamp and metadata commit count remain tied to the original history; excluded recent automation cannot make human activity look newer. An exclusion can increase concentration by removing a contributor. Filtered risk is not inherently more accurate: validate heuristic matches and compare with the baseline. Effective filters and excluded-evidence counts accompany every JSON report.
