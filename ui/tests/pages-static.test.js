...
test('Pages entrypoint uses relative static assets and contains public-demo actions',async()=>{
 ...
 assert.match(html,/View on GitHub/);assert.match(html,/Reset to eShop Sample/);assert.match(html,/dotnet\/eShop · validation sample/);assert.match(html,/Open saved JSON report…/);assert.match(html,/Jump to eShop sample/);assert.match(html,/continuity analyze/);assert.match(html,/data-tooltip="Choose a saved JSON report/);assert.match(html,/data-tooltip="Download the report currently shown here/);assert.match(html,/aria-describedby="open-report-help"/);assert.match(html,/aria-describedby="export-report-help"/);assert.match(html,/id="open-report-help"/);assert.match(html,/id="export-report-help"/);assert.equal((html.match(/To inspect your own result/g)||[]).length,1);assert.match(html,/tabindex="0"/);assert.match(app,/label\[for="upload"\]/);assert.doesNotMatch(html,/Explore Demo|Open Analysis Report/);assert.match(html,/Export Analysis Report/);assert.match(html,/Analyze Your Repository/);assert.match(html,/Your report stays in your browser/);
 assert.match(html,/<section id="local-explorer"[^>]*hidden>/);
 assert.match(html,/<button id="components-tab" class="active" aria-pressed="true">Components<\/button>/);assert.match(html,/<section id="overview" hidden>/);assert.match(html,/<section id="components">/);
