...
function load(data,name,demo=false,sourceType='LOCAL JSON'){report=validateReport(data);selected=report.components[0]?.component;activePerson=null;$('search').value='';$('risk').value='';$('source-name').textContent=name;$('source-type').textContent=demo?'SYNTHETIC DATA':sourceType;$('message').textContent=report.shallow?'This report was created from a shallow clone; incomplete history can affect the results.':'';render();}
...
async function checkLocalExplorer(){try{const response=await fetch('/api/health');if(!response.ok)throw Error();$('local-status').textContent='Local explorer connected. Repository analysis stays on this computer.';for(const id of ['run-local-analysis','run-clone-analysis'])$(id).disabled=false;}catch{$('local-status').textContent='Start this interface with continuity explorer to analyze a repository here.';for(const id of ['run-local-analysis','run-clone-analysis'])$(id).disabled=true;}}
async function runAnalysis(inputId,buttonId,sourceType,emptyMessage){const repository=$(inputId).value.trim();if(!repository){$('message').textContent=emptyMessage;return;}const button=$(buttonId);button.disabled=true;button.textContent='Analyzing…';$('message').textContent='';try{const response=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({repository})});const data=await response.json();if(!response.ok)throw Error(data.error||'Analysis failed.');load(data,repository,false,sourceType);}catch(error){$('message').textContent=error instanceof Error?error.message:'Analysis failed.';}finally{button.disabled=false;button.textContent=buttonId==='run-clone-analysis'?'Clone and analyze':'Analyze path';}}
$('run-local-analysis').onclick=()=>runAnalysis('repository-path','run-local-analysis','LOCAL GIT','Enter a local Git repository path.');
$('run-clone-analysis').onclick=()=>runAnalysis('clone-url','run-clone-analysis','REMOTE GIT','Enter a public HTTPS clone URL.');
checkLocalExplorer();
render();
