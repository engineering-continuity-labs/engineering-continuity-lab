...
async function checkLocalExplorer(){try{const response=await fetch('/api/health');if(!response.ok)throw Error();$('local-status').textContent='Local explorer connected. Repository analysis stays on this computer.';$('run-analysis').disabled=false;}catch{$('local-status').textContent='Start this interface with continuity explorer to analyze a repository here.';$('run-analysis').disabled=true;}}
$('run-analysis').onclick=async()=>{const repository=$('repository-path').value.trim();if(!repository){$('message').textContent='Enter a local Git repository path.';return;}const button=$('run-analysis');button.disabled=true;button.textContent='Analyzing…';$('message').textContent='';try{const response=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({repository})});const data=await response.json();if(!response.ok)throw Error(data.error||'Analysis failed.');load(data,repository,false,'LOCAL GIT');}catch(error){$('message').textContent=error instanceof Error?error.message:'Analysis failed.';}finally{button.disabled=false;button.textContent='Analyze repository';}};
checkLocalExplorer();
render();
